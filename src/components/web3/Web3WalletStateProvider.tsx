...
import { getConnectorInfo, getAddChainParameters, ConnectorId, ConnectorsData, wallets } from 'components/web3/Web3Types'
...
const useWeb3WalletState = (connectorsData: Record<ConnectorId, { id: ConnectorId; name: string; connector: Connector }>) => {
    ...
    const activate = async (connectorId: ConnectorId, _chainId?: number) => {
        ...
        if (!_chainId) await _connector.deactivate()
        await _connector.activate(_wallet !== wallets.metaMask ? Config.chains[0] : !_chainId ? undefined : getAddChainParameters(_chainId))
        contractCaller.current = _connector.provider ? new ContractCaller(new Web3Provider(_connector.provider as any)) : null
        Config.web3.contractCaller = contractCaller.current
    }
    ...
    return {
        ...
        provider,
        // balance,
        contractCaller: contractCaller.current,
        getBalance,
    }
}
